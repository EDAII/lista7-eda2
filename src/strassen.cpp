...
double* EMSCRIPTEN_KEEPALIVE myFunction(double *matrixA,int line_matrixA,int columns_matrixA,
                                        double *matrixB,int line_matrixB,int columns_matrixB){
    
    double A[SIZE][SIZE], B[SIZE][SIZE], C[SIZE][SIZE];
    double *Z = (double*)malloc(sizeof(double)*(line_matrixA*line_matrixA));
    int cont_aux = 0;
    for(int cont_line =0;cont_line<line_matrixA;cont_line++){
        for(int cont_column =0;cont_column<columns_matrixA;cont_column++){
            A[cont_line][cont_column] = matrixA[cont_aux];
            B[cont_line][cont_column] = matrixB[cont_aux];
            cont_aux++;
        }
    }
    printf("\n\n\n\n\n");
    ...
    int size_aux = line_matrixA*line_matrixA;
    double vec_aux[size_aux];

    cont_aux =0;
    for(int cont_line =0;cont_line<line_matrixA;cont_line++){
        for(int cont_column =0;cont_column<columns_matrixA;cont_column++){
            Z[cont_aux] = C[cont_line][cont_column];
            cont_aux++;
        }
    }
